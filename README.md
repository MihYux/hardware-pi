...
当前 0.6.2 已完成正式版核心链路的无 Electron 迁移：
...
保存两个访问令牌后，页面会分别验证设备接口与管理接口，并明确提示设备令牌错误、管理令牌错误或 Pi 无响应。连接请求 8 秒无响应会自动退出“正在连接”，可在连接设置中直接重新验证相同令牌。
...
网页能打开但令牌验证失败时，重新从 Pi 的 `.env` 复制完整令牌，避免带入空格：

```bash
grep '^HARDWARE_PI_.*TOKEN=' .env
docker compose ps
docker compose logs --tail=100 hardware-pi
```
